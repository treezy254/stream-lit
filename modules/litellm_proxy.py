...
def start_litellm_proxy(log_file_path, config_file_path, append_to_log=False):
    def run_process():
        with open(log_file_path, "a" if append_to_log else "w") as log_file:
            # Read the model name from the config file
            with open(config_file_path, "r") as config_file:
                config_data = yaml.safe_load(config_file)
                model_name = config_data.get('model_list', [])[0].get('model_name', '')  # Assuming the first model in the list

            if model_name:
                litellm_command = ["litellm", "--model", f"ollama/mixtral", "--drop_params"]
                # Start the litellm process
                process = subprocess.Popen(litellm_command, stdout=log_file, stderr=subprocess.STDOUT,)
                process.communicate()
            else:
                print("Error: Model name not found in the config file.")
    ...
    try:
        result = subprocess.run(
            ["curl", "--location", "http://127.0.0.1:8000/chat/completions",
             ...
             "--data", '{"model": "ollama/mixtral", "messages": [{"role": "user", "content": "what llm are you"}]}'],
